...
function buildForm() {
  ...
  TASKS_RONDELLE.forEach(function(task, idx) {
    ...
    appendTaskRow(tasksSec, task.id, task.machine, task.qui, tv, color);
  });
...
async function saveSession() {
  ...
  // Vider formulaire apres affichage
  document.getElementById("f-machine-name").value = "";
...
function renderGantt(date, machine, data) {
  ...
  ["grand_t1","petit_t1","rondelle"].forEach(function(key) {
    var t=targets[key]||{}; regT(t.sh||"",t.sm||"",t.eh||"",t.em||"");
  });
  TASKS_RONDELLE.forEach(function(task) {
    var t=tasks[task.id]||{}; regT(t.sh||"",t.sm||"",t.eh||"",t.em||"");
  ...
  if (!isFinite(minT)) minT=360;
  if (!isFinite(maxT)) maxT=minT+120;
  ...
  TASKS_RONDELLE.forEach(function(task,idx) {
    ...
    allTaskRows.push({type:"fixed",task:task,t:t,idx:idx,sMin:toMin(start)});
  });
  extras.forEach(function(et,idx) {
    var start=getTV(et.sh||"",et.sm||"");
    allTaskRows.push({type:"extra",et:et,idx:idx,sMin:toMin(start)});
  });
  ...
  allTaskRows.forEach(function(rowData, rowIdx) {
